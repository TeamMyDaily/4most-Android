...
class GoalViewModel: BaseViewModel() {
    ...
    fun getGoalData() {
        /* 임시 데이터 */
        val tempList = listOf(
            Goal("아웃풋", "블로그에 1개 이상 퍼블리싱 하기", isGoalExist = true, isGoalCompleted = true),
            Goal("열정", null, isGoalExist = false, isGoalCompleted = false),
            Goal("경청", null, isGoalExist = false, isGoalCompleted = false),
            Goal("선한영향력", "거짓말 치지 않고 선하게 살기", true, isGoalCompleted = false)
        )
